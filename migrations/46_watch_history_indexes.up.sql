...
CREATE INDEX CONCURRENTLY IF NOT EXISTS watch_history_user_unwatched_idx
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS watch_history_user_watched_resource_idx
	ON public.watch_history (user_id, resource_id)
	WHERE watched = true;

-- Episode lookup by resource_id (no index existed on the FK column).
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_episode_resource_id
