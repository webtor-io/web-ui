...
- **Onboarding checklist tier (dev-only)** — `?onboarding=free` / `?onboarding=paid` on the home page renders the activation checklist as that tier, so the locked PRO rows can be reviewed without a second account. Gated by `gin.Mode() != gin.ReleaseMode` in `services/onboarding/tier.go`; affects rendering only. Because the tier is decided in the resolver, it also switches the navbar counter on any page. See `docs/onboarding.md`.
