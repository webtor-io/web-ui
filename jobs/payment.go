...
const (
	...
	// paymentPollTimeout bounds the watch job, not the payment: the IPN
	// grants the tier regardless, the page just stops watching.
	paymentPollTimeout = 20 * time.Minute
)
...
func (s *Jobs) PaymentStatus(c *web.Context, pc *np.Client, paymentID string) (*job.Job, error) {
	ctx, cancel := context.WithTimeout(context.Background(), paymentPollTimeout)
