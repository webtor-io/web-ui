...
- **Status badge** — `<span data-vault-progress-badge>` renders a loading-dots fallback for SSR/no-JS; JS swaps it for a live badge `<state> <pct>%` (with peer count when relevant). Once `state == "vaulted"` the badge falls back to `Claimable` and the SSE source closes. Every badge in the column is `whitespace-nowrap` and the column is `w-36 sm:w-44`: a two-word label ("Waiting for seeders", "Caching paused") must never fold into two lines inside the fixed-height badge (2026-09-03).
