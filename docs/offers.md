...
| Player's buffering label (`lib/player/BufferingLabel.jsx`: the lock "Buffering \| [lock] 5 Mbps ›" on the player's "Buffering" pill, and the card it opens over the video, inside the player so fullscreen keeps it; docs/player.md "Buffering label") | exactly where the transfer status plan box above sells the stream box at a stall (`stream_stall` with its box: the server's verdict at the cap with the box due, something faster on sale, a real stall, not inside the grace window, not while the grace popup is up or on its way, no answered offer standing — `lib/transferStatus.js` `playerLabel` asks `present()`), and only while the player shows its buffering label for a stall of the playing film (not a start, a session seek, the translation hold or the next file loading) and, by its own clock, past the grace window; never in an embed (no status there); the card closes on its ×, when the film plays again or when the status takes its word back | the lock: the viewer's own cap; the card: the stream box's own words ("the video loads slower than it plays", the stream job's "… and this file needs 8.7 Mbps", "watch without the speed cap", the trial note) — no copy of its own | `player-label-lock-shown` (the lock drawn, once per player), `donate-player-label-shown` (the card opened), `donate-player-label` (its button), props as the status box's with `location` player | `player-label` (free only; built in Go next to the status box's link, `Plan.Player.URL`, carried over the status SSE) |
...
`promo-banner`, `download-nudge`, `limit-modal`, `status-bar`, `player-label`, `grace`,
`no-peers`, `onboarding`, `donate`. The query string is client-supplied, so anything else is counted as `other`
