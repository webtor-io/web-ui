...
- `templates/views/action/stream_video.html` + `stream_audio.html` — `data-grace-duration-sec` attribute on player tag; `#grace-cta` popup on video page — a native `<dialog class="modal">` opened with `showModal()` like the player's captions and embed dialogs (2026-09-26): the top layer, so nothing clips it (the absolutely placed card before it was cut by the player's rounded `overflow:hidden` frame), centred on the screen, shown over a fullscreen player without leaving fullscreen; Esc is the close (the answer "dismiss"), focus starts on "continue at N Mbps" (title + body + the promo plan's CTA — `offer.keepFullSpeed` with `offer.trialNote` under it, rendered only when there is a catalog to sell from — + `Continue at <rate> Mbps` secondary + dismiss X)
