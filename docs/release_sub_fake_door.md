...
Статус: **завершён 2026-05-24**, результаты — см. раздел [Результаты](#результаты-2026-05-24).
Дата фиксации плана: 2026-05-20.
Дата запуска: 2026-05-20.
Дата остановки рендера: 2026-05-24 (см. `handlers/resource/release_subscribe_banner.go`).
...
## Результаты (2026-05-24)

Окно сбора: 2026-05-20 → 2026-05-23 (4 суток), webtor.io.
Метод: dedupe по `session_id` (per spec); CTR = `uniq_yes / uniq_shown`.

### Aggregate

| Сегмент | uniq_shown | uniq_yes | CTR |
|---|---|---|---|
| **All** | 2,021 | 45 | **2.2%** |
| **Free** | 1,874 | 25 | **1.3%** |
| **Paid (silver+bronze+gold)** | 147 | 20 | **13.6%** |

Aggregate CTR 2.2% — **ниже 5%-порога decision gate** → для широкой выборки гипотеза опровергнута.

### По tier

| Tier | uniq_shown | uniq_yes | uniq_no | uniq_dismissed | CTR |
|---|---|---|---|---|---|
| free | 1,874 | 25 | 32 | 113 | **1.3%** |
| silver | 104 | 12 | 4 | 5 | **11.5%** |
| bronze | 39 | 7 | 0 | 5 | **17.9%** |
| gold | 4 | 1 | 0 | 0 | 25% (N=4, ориентировочно) |

silver/bronze попадают в "10-20% маргинальный" квадрант gate — но **разрыв с free в 9-14×** делает интерпретацию однозначной: фича работает там, где люди уже платят, и не работает там, где не платят.

### Выводы

1. **Гипотеза "release-level subscription" подтверждена для платной аудитории, опровергнута для free.** Поведенческая модель free-юзера на webtor — transactional (пришёл, скачал/посмотрел, ушёл). Future-engagement signaling его не цепляет. Платник, наоборот, ведёт себя как retention-сегмент: ему интересно «следить» за раздачей.
2. **Это не специфика баннера, а класс фичи.** Любая future-engagement функция (push о новой серии, email-digest, watchlist-alert, «вернуться к раздаче когда обновится») — той же природы. Дефолтный rollout таких фич = paid-only или tier-gated.
3. **Decision gate работает на агрегате, но требует tier-segmentation чтобы не убить продуктивную полу-выборку.** Если бы решали только по 2.2% — выбросили бы фичу, которая показывает 11-18% у платников. Записать в playbook: при fake-door на смешанной аудитории gate всегда сегментируем по tier.
4. **CTA "Не нужно" vs "Закрыть" (free)**: 32 vs 113 — закрывают в 3.5× чаще, чем явно отказываются. Баннер для free не раздражает, он **неинтересен**. Это слабее, чем "раздражает" — повторный fake-door с другим UX не поможет.
5. **Anon-показы**: в данных tier нет колонки "anon" отдельно (free перекрывает анонов в трекинге, см. рендер `data-is-anon`). Detailed anon→registration воронка не отделена, secondary CTA «зарегистрируйся» не выделен — оставлено как gap, не критично для решения.

### Следующие шаги (план для paid-feature)

Не делать сейчас, но зафиксировать как следующую тему, когда вернёмся к retention-кластеру:

- **Push/email digest "новая серия раздачи"** как perk silver+. Фактическая фича, не fake-door.
- Скоуп подписки: `release × season` (как в исходном плане).
- Триггер: не баннер на странице раздачи (CTR 13% — есть, но интрузивно для платника, который и так знает что хочет), а **opt-in в settings** + автоматическая подписка при `add-to-library` для airing-сериалов (mark sane defaults).
- Доставка: web push (best-effort, бесплатно) + email digest раз в неделю (один блок «что обновилось из твоих раздач»).
- Парсер-фиксы (trailing `)`, indexer suffix, audio-channel prefix) — нужны под matching engine; делаем в том же спринте.

### Что было удалено / отключено

- Рендер баннера отключён в `prepareReleaseSubscribeBanner` (возврат `nil`, см. файл).
- Шаблон `templates/partials/resource/release_subscribe_banner.html`, JS `assets/src/js/app/resource/release_subscribe_banner.js`, страновые ключи `release_sub.*` оставлены — это потенциальный scaffold, если решим вернуть fake-door под другим cohort/copy. Удалять отдельным PR, если scaffold не пригодится в течение месяца.
