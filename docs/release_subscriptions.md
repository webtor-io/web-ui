...
Umami за 30 дней: `release-sub-created` 94 события без `source` (это баннер —
атрибутные события не несли property, исправлено 02.09: `data-umami-event-source`),
empty_streams 41, discover_season 26, empty_filters 11, prefs_miss 4;
`release-sub-login-click` **86 (81 сессия)** — анонимы жмут «подписаться» на
баннере, уходят в логин, а после возврата должны кликнуть снова: при 30
подписках с баннера это главная утечка точки входа. Закрыто 02.09: ссылка
анонима несёт `?release_sub=1` в return-url, а `release_subscribe_banner.js`
после возврата сам отправляет форму подписки (POST с CSRF; параметр снимается
через replaceState до сабмита, чтобы перезагрузка не подписала дважды).
Umami: `release-sub-created` с `resumed=1`. `release-sub-limit-hit` — 6
сессий упёрлись в лимит free (3).
...
**Разовая зачистка.** `subscription poll --subscription-sweep-finished-seasons`
(env `RELEASE_SUB_SWEEP_FINISHED_SEASONS=true`) вместо опроса проходит все
незавершённые сезонные подписки, спрашивает `IsAiringSeasonChecked` и
завершает те, чей сезон уже не выходит: досылает накопившиеся релизы, шлёт
письмо «сезон завершён» и ставит `completed` — тот же путь, что у обычного
завершения в поллере. Ответ «не смог проверить» строку не трогает. Запуск —
разовый Job из CronJob `web-ui-subscription-poll` с этой переменной; нужен
после того, как правило подписки перевели на сезон (до этого писались строки
на завершённые сезоны возвращающихся сериалов).
