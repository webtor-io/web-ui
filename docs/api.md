...
| `GET` | `/library` | `api:read` | Your torrents (`type=all\|movies\|series`; `sort=recent\|name\|year\|rating` and `watched=all\|watched\|unwatched` — year/rating/watched need a movies/series section; `limit`, `offset`) |
